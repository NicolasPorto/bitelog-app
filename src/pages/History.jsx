...
export default function History({ setPage, lang }) {
  ...
  const exportNutriReport = () => {
    // Cabeçalhos traduzidos para o CSV
    ...
    <div className="min-h-[100dvh] bg-background flex flex-col text-foreground pb-24">
      <header className="sticky top-0 z-20 bg-background/80 backdrop-blur-lg border-b border-border/50 safe-top">
        <div className="max-w-md mx-auto px-6 py-4 flex items-center justify-between">
          <h1 className="text-xl font-bold tracking-tight">{t.history_title}</h1>
          <button
            onClick={exportNutriReport}
            className="flex items-center gap-1.5 text-xs font-semibold text-primary bg-primary/10 hover:bg-primary/20 px-3 py-1.5 rounded-full transition-colors active:scale-95"
          >
      ...
      <main className="flex-1 max-w-md mx-auto w-full px-4 py-4 space-y-4">
        ...
        {historyGrouped.map((day) => {
          ...
          return (
            <div
              key={day.date}
              className={`rounded-xl overflow-hidden border transition-all ${isToday ? "bg-gradient-to-br from-primary/10 to-accent/5 border-primary/20" : "bg-card border-border"}`}
            >
              <button
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="w-11 h-11 rounded-xl bg-muted/60 flex items-center justify-center shrink-0">
                    <CalendarDays
                      className={`w-5 h-5 ${isToday ? "text-primary" : "text-muted-foreground"}`}
                    />
                  </div>
